...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--image_size',
                        ...
                        type=int,
                        default=224,
                        help='Input images will be resized to --image_size x --image_size (square). Code only tested with 224x224, so no guaruantees that it works for different sizes.')
    ...
    parser.add_argument('--randseedother',
                        type=int,
                        default=1,
                        help='Random seed')
    parser.add_argument('--randseeddata',
                        type=int,
                        default=1,
                        help='Random seed')
    ...
    parser.add_argument('--extra_test_image_folder',
                        type=str,
                        default='./experiments',
                        help='Folder with images that PIP-Net will predict and explain, that are not in the training or test set. E.g. images with 2 objects or OOD image. Images should be in folder. E.g. ./experiments/images/')
    parser.add_argument('--datasplit',
                        type=str,
                        default='',
                        help='Type of datasplit to use, i.e. custom or official')
    parser.add_argument('--SIL_csvfilepath',
                        type=str,
                        default='',
                        help='Input file containing the groundtruth labels, image names and location')
    parser.add_argument('--preprocessed_imagepath',
                        type=str,
                        default='',
                        help='Directoy where the input images are stored')
    parser.add_argument('--usevalidation',
                        action='store_true',
                        help='Flag that indicates whether to use validation set for early stopping')
    parser.add_argument('--numclasses',
                        type=int,
                        default=1,
                        help='Number of classes')
    parser.add_argument('--patienceepochs',
                        type=int,
                        default=0,
    ...
    parser.add_argument('--early_stopping_criteria',
                        type=str,
                        default='auc',
                        help='metric to be used for early stopping')
    parser.add_argument('--valscore_resumetrain',
                        type=float,
                        default=None,
                        help='best validation score from early stopping. This is used for resuming training of models that use early stopping criterion. Corresponds to the metric used for early stopping.')
    parser.add_argument('--flipimage',
                        action='store_true',
                        help='Flag that indicates whether the right sided image will be flipped horizontally to keep all images at the same side during training')
    parser.add_argument('--classtype',
                        type=str,
                        default='diagnosis',
                        help='Type of label to use for model training; If diagnosis, then malignant and benign classes are used, if birads, then 1 to 5 are used.')
    parser.add_argument('--modelid',
                        type=int,
                        default=1,
                        help='unique id for each model')
    parser.add_argument('--scheduler_net',
                        type=str,
                        default='',
                        help='scheduler for the feature extractor backbone')
    parser.add_argument('--align_pf_weight',
                        type=float,
                        default=5.0,
                        help='alignment loss weight')
    parser.add_argument('--tanhloss_weight',
                        type=float,
                        default=2.0,
                        help='tanh loss weight')
    parser.add_argument('--classloss_weight',
                        type=float,
                        default=2.0,
...
def save_args(args: argparse.Namespace, directory_path: str) -> None:
    ...
    with open(directory_path + '/args'+'_'+str(args.randseedother)+'_'+str(args.randseeddata)+'.txt', 'w') as f:
    ...
    with open(directory_path + '/args'+'_'+str(args.randseedother)+'_'+str(args.randseeddata)+'.pickle', 'wb') as f:
        pickle.dump(args, f)                                                                               
...
def get_optimizer_nn(net, args: argparse.Namespace) -> torch.optim.Optimizer:
    ...
    torch.cuda.manual_seed_all(args.randseeddata)
