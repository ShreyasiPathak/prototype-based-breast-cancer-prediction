...
def load_model(net, path):
    ...
    print(checkpoint['epoch_finetune'])
...
def class_specific_score(args):
    ...
    feature_net, add_on_layers, pool_layer, classification_layer, num_prototypes = get_network(2, args)
    # Create a PIP-Net
    ppnet = PIPNet(num_classes=2,
    ...
    match_count = 0
    total_count = 0
    print("purity count:", purity_file.shape[0])
    for i in range(0, purity_file.shape[0]):
        ...
        print("prototype:", i, protoid, protoname, flush=True)
        wt_benign = ppnet.module._classification.weight[0, protoid].item()
        wt_malign = ppnet.module._classification.weight[1, protoid].item()
        ...
        print("wt benign, malign:", wt_benign, wt_malign, flush=True)
        if not dataset_category_row.empty:
            if wt_benign!=0 or wt_malign!=0:
                total_count+=1
    ...
    print("match count average:", match_count, total_count, match_count/total_count, flush=True)
