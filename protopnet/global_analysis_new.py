...
from settings import base_architecture, img_size, prototype_shape, num_classes, num_train_epochs, num_warm_epochs, \
                    lrdecay_gamma, prototype_activation_function, add_on_layers_type, coefs, lrscheduler, \
                    joint_optimizer_lrs, joint_lr_step_size, warm_optimizer_lrs, last_layer_optimizer_lr, \
                    randseeddata, randseedother 

def load_model(net, path):
    checkpoint = torch.load(path, map_location='cuda:0')
    net.load_state_dict(checkpoint['model_state_dict'])
    print(checkpoint['epoch_joint'])
    print(checkpoint['epoch_warm'])
    return net
...
#print model details
log("num prototypes {0}".format(ppnet.module.num_prototypes))
img_size = ppnet.module.img_size
log("img size: {0}".format(img_size))
proto_percentile = 0.999 #0.95
log("proto percentile: {0}".format(proto_percentile))
...
#check test accuracy
accu = tnt.test(model=ppnet, dataloader=test_loader_normalize, class_specific=True, coefs=coefs, device=device)
