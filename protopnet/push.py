...
def push_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for push_iter, (_, search_batch_input, search_y, _) in enumerate(dataloader):
        '''
    ...
    if proto_epoch_dir != None and proto_bound_boxes_filename_prefix != None:
        np.save(os.path.join(proto_epoch_dir, proto_bound_boxes_filename_prefix + '-receptive_field' + str(epoch_number) + '.npy'),
                proto_rf_boxes) # receptive fields of the most similar patch among all images in the training set
        np.save(os.path.join(proto_epoch_dir, proto_bound_boxes_filename_prefix + str(epoch_number) + '.npy'),
                proto_bound_boxes) # boundary of the highest activated patch extracted from the percentile method
...
def update_prototypes_on_batch(search_batch_input,
    ...
    with torch.no_grad():
        ...
        #print("shape of last layer:", protoL_input_torch.shape)

    protoL_input_ = np.copy(protoL_input_torch.detach().cpu().numpy())
    proto_dist_ = np.copy(proto_dist_torch.detach().cpu().numpy())
    #print("proto dist:", proto_dist_.shape) #16, 400, 25, 25
    ...
    for j in range(n_prototypes):
        ...
        batch_min_proto_dist_j = np.amin(proto_dist_j) # what is the minimum distance of the prototype with all the locations in the feature map across all images in the batch
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            batch_argmin_proto_dist_j = \
                list(np.unravel_index(np.argmin(proto_dist_j, axis=None),
                                    proto_dist_j.shape)) # what is the location of the minimum distance across the batch
            if class_specific:
                '''
            ...
            #print("search batch:",search_batch.shape)
            #print("batch argmin proto dist j:", batch_argmin_proto_dist_j)
            #print("protoL rf info:", protoL_rf_info)
            rf_prototype_j = compute_rf_prototype(search_batch.size(2), batch_argmin_proto_dist_j, protoL_rf_info)
            
            # get the whole image
            original_img_j = search_batch_input[rf_prototype_j[0]]
            original_img_j = original_img_j.numpy()
            #print('original image shape:', original_img_j.shape) #3, 1536, 768
            original_img_j = np.transpose(original_img_j, (1, 2, 0))
            #print('original image shape:', original_img_j.shape)
            original_img_size = [original_img_j.shape[0], original_img_j.shape[1]]
            #print('original image shape:', original_img_size)
            ...
            # find the highly activated region of the original image. Distance to similarity function.
            proto_dist_img_j = proto_dist_[img_index_in_batch, j, :, :]
            #the term inside the log will always generate values>1. np.log will convert it to values between 0 and 1. 
            #Larger distance has smaller values using the term inside log, resulting in smaller values with np.log. Thus, larger distances have lower similarity
            ...
            if dir_for_saving_prototypes is not None:
                ...
                if prototype_img_filename_prefix is not None:
                    ...
                    #print(heatmap.shape)
                    heatmap = heatmap[...,::-1] #heatmap contains values in BGR from cv2.applycolormap. The line reverses the values in the last dimension to convert it to RGB.
