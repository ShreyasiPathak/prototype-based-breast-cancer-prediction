...
def push_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    prototype_selection_notgreedy(prototype_network_parallel, min_each_image_proto_info, n_prototypes, global_min_proto_dist, global_min_proto_info)
...
def store_prototypes_on_batch(search_batch_input,
    ...
    for j in range(n_prototypes):
        ...
        min_idx = np.argmin(proto_dist_j.reshape(proto_dist_j.shape[0], -1), axis=1) #min patch location in each image
        unravel_min_idx = list(np.unravel_index(min_idx, proto_dist_j[0,:,:].shape)) #converting flattened patch location to 2D x, y location
        ...
        batch_img_idx = np.array(class_to_img_index_dict[target_class]) + start_index_of_search_batch #image idx of all images in the batch in the whole training set
        batch_min_each_image_info_proto_j = np.array(list(zip(batch_img_idx, batch_min_proto_dist_j, unravel_min_idx[0], unravel_min_idx[1]))) #zipping information of the img idx, the distance of the prototype to the min patch and the min patch location
...
def prototype_selection_notgreedy(prototype_network_parallel, min_each_image_proto_info, n_prototypes, global_min_proto_dist, global_min_proto_info):
    images_assigned_idx = {0:[], 1:[]}
    #print("min each image proto info:", min_each_image_proto_info)
    #print("min each image proto info proto 0 shape:", min_each_image_proto_info['0'].shape)
    for j in range(n_prototypes):
