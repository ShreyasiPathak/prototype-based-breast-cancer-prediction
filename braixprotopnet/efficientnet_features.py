...
class MBConv(nn.Module):
    def __init__(
        ...
        if expanded_channels != cnf.input_channels:
            ...
            self.block_kernel_sizes.extend([1])
            self.block_strides.extend([1])
            self.block_paddings.extend([(1 - 1) // 2 * 1])
        ...
        self.block_kernel_sizes.extend([cnf.kernel])
        self.block_strides.extend([cnf.stride])
        self.block_paddings.extend([(cnf.kernel - 1) // 2 * 1])

        # squeeze and excitation
        squeeze_channels = max(1, cnf.input_channels // 4)
        layers.append(se_layer(expanded_channels, squeeze_channels, activation=partial(nn.SiLU, inplace=True)))
        self.block_kernel_sizes.extend([1, 1])
        self.block_strides.extend([1, 1])
        self.block_paddings.extend([0, 0])
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.kernel_sizes.extend([3])
        self.strides.extend([2])
        self.paddings.extend([(3 - 1) // 2 * 1])
        ...
        for cnf in inverted_residual_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                block_call = block_cnf.block(block_cnf, sd_prob, norm_layer)
        ...
        self.kernel_sizes.extend([1])
        self.strides.extend([1])
        self.paddings.extend([(1 - 1) // 2 * 1])
    ...
    def conv_info(self):
        #print("kernel size:", self.kernel_sizes)
...
def efficientnet_b0_features(pretrained=False, **kwargs):
    model = efficientnet_b0(pretrained=pretrained, weights=EfficientNet_B0_Weights.DEFAULT)
    with torch.no_grad():
