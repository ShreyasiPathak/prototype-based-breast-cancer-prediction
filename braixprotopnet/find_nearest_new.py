...
class ImagePatch:
    ...
    def __init__(self, patch, label, distance,
                 original_img=None, act_pattern=None, patch_indices=None, imgname=None):
        self.patch = patch
        self.label = label
...
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
                                         ...
                                         root_dir_for_saving_images='./nearest_protopshare',
                                         log=print,
    ...
    for idx, (_, search_batch_input, search_y, _) in enumerate(dataloader):
            ...
            for j in range(n_prototypes):
                ...
                if full_save:
                    ...
                    if prototype_network_parallel.module.prototype_activation_function == 'log':
                        ...
                        temp = 128.
                        act_pattern = np.exp(-distance_map[j]/temp)
                    ...
                    imgname = df.iloc[idx*batch_size+img_idx]['ShortPath']
                    #imgname = dataloader.dataset.imgs[idx*dataloader.batch_size+img_idx][0]
    ...
    with open(os.path.join(root_dir_for_saving_images,'protopnet_cbis_topk.csv'), "w") as csvfile:
        ...
        for j in range(n_prototypes):
            ...
            if full_save:
                ...
                for i, patch in enumerate(heaps[j]):
                    ...
                    img_size1 = patch.original_img.shape[1]
                    upsampled_act_pattern = cv2.resize(patch.act_pattern,
                                                    dsize=(img_size1, img_size),
                    ...
                    '''if patch.patch.shape[0] != img_size or patch.patch.shape[1] != img_size1:
                        np.save(os.path.join(dir_for_saving_images,
                    ...
                    high_act_patch_indices = find_high_activation_crop(upsampled_act_pattern, percentile = 99.9)
                    ...
                    '''
                    writer.writerow([j, patch.imgname, high_act_patch_indices[0], high_act_patch_indices[1], high_act_patch_indices[2], high_act_patch_indices[3]])
                    # TODO how to get imagename
